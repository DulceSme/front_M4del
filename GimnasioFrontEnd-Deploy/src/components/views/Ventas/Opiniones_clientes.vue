<template>
    <div>
      ...
      <table class="min-w-full bg-white mt-5">
        ...
        <tbody>
          <tr v-for="program in programs" :key="program.Descripcion">
            ...
            <td class="border px-4 py-2">
              <button @click="editProgram(program.Descripcion)" class="bg-yellow-500 text-white px-4 py-2 rounded">
                Editar
              </button>
              <button @click="deleteProgram(program.Descripcion)" class="bg-red-500 text-white px-4 py-2 rounded ml-2">
                Eliminar
              </button>
            </td>
          </tr>
...
export default {
  data() {
    return {
      programs: [
        { Descripcion: "Programa de Nutrición", Tipo: "Activo", Respuesta: "Buena", Estatus: "Activo", Atencion: "Excelente", Registro: "2024-01-15", Actualizacion: "2024-08-01" },
        { Descripcion: "Entrenamiento Funcional", Tipo: "Inactivo", Respuesta: "Moderada", Estatus: "Inactivo", Atencion: "Adecuada", Registro: "2024-03-10", Actualizacion: "2024-07-25" },
        { Descripcion: "Yoga Avanzado", Tipo: "Activo", Respuesta: "Excelente", Estatus: "Activo", Atencion: "Muy Buena", Registro: "2024-02-05", Actualizacion: "2024-08-10" }
      ],
      newProgram: {
        Descripcion: "",
        Tipo: "",
  ...
  methods: {
    ...
    editProgram(Descripcion) {
      this.currentProgram = { ...this.programs.find(p => p.Descripcion === Descripcion) };
      this.editingProgram = true;
    },
    updateProgram() {
      const index = this.programs.findIndex(p => p.Descripcion === this.currentProgram.Descripcion);
      if (index !== -1) {
    ...
    deleteProgram(Descripcion) {
      this.programs = this.programs.filter(p => p.Descripcion !== Descripcion);
    }
  }
};
</script>
